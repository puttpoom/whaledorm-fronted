          ...
          <div className="grid grid-rows-4 items-center gap-[8px] p-4">
            ...
            <div className="inline-flex gap-2 items-center">
              <VacantRoomLable bgColor="green">
                ...
                มีห้องว่างจำนวน{" "}
                {dorm.totalVacantRoom || mockData.vancantNumbers} ห้อง
              </VacantRoomLable>
              <span className="text-sm text-gray-600">
                {/* {findNewestCreatedAt(dorm.room.createdAt)} */}
                {dorm.createdAt || mockData.createdAt}
              </span>
            </div>
            <div className="text-red-500 font-bold text-[20px]">
              {min(dorm.room)}{" "}
              {dorm.room.length > 1 ? "-" + " " + max(dorm.room) : null}{" "}
              บาท/เดือน
            </div>
          </div>
        </div>
